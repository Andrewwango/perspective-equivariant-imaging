...
def make_dataloaders(
    dataset_name: str, physics: dinv.physics.Physics, device="cpu"
) -> tuple:
    ...
    if dataset_name == "spacenet":
        transform, img_loader, is_valid_file = (
            spacenet_transform(),
            spacenet_loader,
            spacenet_is_valid_file(),
        )

    dataset_path = dinv.datasets.generate_dataset(
        train_dataset=ImageFolder(
            f"data/{dataset_name}/subset/train",
            transform=transform,
            loader=img_loader,
            is_valid_file=is_valid_file,
        ),
        test_dataset=ImageFolder(
            f"data/{dataset_name}/subset/test",
            transform=transform,
            loader=img_loader,
            is_valid_file=is_valid_file,
        ),

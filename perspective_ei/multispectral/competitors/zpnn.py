...
class ZPNN_CorrelationLoss(nn.Module, MultispectralUtils):
    ...
    def forward(self, y, x_net, physics: Pansharpen, *args, **kwargs):
        ...
        one_minus_rho = 1.0 - torch.clamp(
            xcorr_torch(hrms, pan, half_width=ceil(window_size / 2)), min=-1
        )

        pan_lp = self.blur(pan)
        lrms_up = self.hrms_from_volume(physics.A_adjoint(y))

        one_minus_rho_max = 1.0 - xcorr_torch(
            pan_lp, lrms_up, half_width=ceil((window_size**2) / 2)
        )
...
def xcorr_torch(
    img_1: torch.Tensor, img_2: torch.Tensor, half_width: int
) -> torch.Tensor:

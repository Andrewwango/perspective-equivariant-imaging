...
class PanganDiscriminator(nn.Module):
    ...
    def __init__(self, input_channels: int = 1):
        ...
        self.layer1 = self.conv_block(
            input_channels, 16, kernel_size=6, stride=4, padding=1
        )
    ...
    def conv_block(
        self, in_channels, out_channels, kernel_size, stride, padding, activation=True
    ):
...
class PanganStructuralLoss(BaseStructuralLoss):
    ...
    def forward(self, y: Tensor, x_net: Tensor, *args, **kwargs) -> Tensor:
        ...
        return nn.functional.mse_loss(self.highpass(pan_recon), self.highpass(pan))
...
class PanganAdvGenLoss(BaseStructuralLoss):
    ...
    def forward(
        ...
        y: Tensor,
        x_net: Tensor,
        D_spectral: nn.Module,
        D_structural: nn.Module,
        **kwargs
    ) -> Tensor:
        ...
        return (
            loss_spectral * self.weight_adv_spectral
            + loss_structural * self.weight_adv_structural
        )
...
class PanganOptimizer:
    ...
    def __init__(
        ...
        optimizer_d_spectral: Optimizer,
        optimizer_d_structural: Optimizer,
...
class PanganScheduler:
    ...
    def __init__(
        ...
        scheduler_d_spectral: LRScheduler,
        scheduler_d_structural: LRScheduler,
    ):
        self.G = scheduler_g
...
class PanganTrainer(Trainer):
    ...
    def backward_pass(self, g, x, y, x_net):
        # Train G
        loss_mc = self.losses[0](
            x=x, x_net=x_net, y=y, physics=self.physics[g], model=self.model
        )
        loss_struct = self.losses[1](
            x=x, x_net=x_net, y=y, physics=self.physics[g], model=self.model
        )
        loss_g_adv = self.losses[2](
            x=x,
            x_net=x_net,
            y=y,
            ...
            D_structural=self.D_structural,
        )
        ...
        loss_d_structural = self.losses[4](
            x=x,
            x_net=x_net,
            y=y,
            physics=self.physics[g],
        ...
        for k, (l, loss) in enumerate(
            zip(
                self.losses,
                [loss_mc, loss_struct, loss_g_adv, loss_d_spectral, loss_d_structural],
            )
        ):

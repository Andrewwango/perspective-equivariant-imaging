...
class SSQBase:
    ...
    def loss_fn(self, pred: Tensor, target: Tensor) -> Tensor:
        ...
        """
        return torch.nn.functional.mse_loss(pred, target) + 1 - cal_ssim(pred, target, gaussian_kernel=False, kernel_size=11)
...
class SSQSpectralLoss(torch.nn.Module, SSQBase, MultispectralUtils):
    ...
    def forward(self, y: Tensor, x_net: Tensor, physics: Pansharpen, *args, **kwargs) -> Tensor:
        lrms = self.lrms_from_volume(y)
        lrms_up = dinv.physics.Downsampling(
            physics.imsize,
            factor=4, 
            filter="bilinear", 
            device=self.device).A_adjoint(lrms)

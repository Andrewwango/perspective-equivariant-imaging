...
def cal_ssim(x_hat: Tensor, x: Tensor, y_channel: bool = False, **kwargs):
    ...
    return structural_similarity_index_measure(x_hat, x, data_range=1.0, **kwargs).detach().cpu().item()
...
class QNR(nn.Module, MultispectralUtils):
    ...
    """
    def __init__(self, physics: Pansharpen, alpha: float = 1, beta: float = 1, p: float = 1, q: float = 1, return_D=False, **kwargs):
        super().__init__()
    ...
    def D_lambda(self, hrms: Tensor, lrms: Tensor) -> float:
        ...
        for b in range(n_bands):
            for c in range(n_bands):
                if b == c: continue #this shouldn't be necessary as 1-1=0 anyway
                out += abs(
                    self.Q(
                        hrms[:, [b], :, :], hrms[:, [c], :, :]
                    ) - self.Q(
